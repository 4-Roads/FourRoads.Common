...
{
	/// <summary>
	/// An abstract base Http Handler for all your <see cref="IHttpHandler"/> needs.
	/// </summary>
	/// <remarks>
	/// <p>
	/// For the most part, classes that inherit from this class do not need to override <see cref="ProcessRequest"/>.
	/// Instead implement the abstract methods and properties and put the main business logic in the <see cref="HandleRequest"/>.
	/// </p>
	/// <p>
	/// HandleRequest should respond with a StatusCode of 200 if everything goes well, otherwise use one of the various
	/// "Respond" methods to generate an appropriate response code.  Or use the HttpStatusCode enumeration if none of these apply.
	/// </p>
	/// </remarks>
	public abstract class BaseHttpHandler : IHttpHandler
	{
		...
		/// </summary>
		/// <remarks>
		/// Return true if this handler does not maintain any state (generally a good practice).  Otherwise returns false.
		/// </remarks>
		...
		/// <summary>
		/// Validates the parameters.  Inheriting classes must implement this and return true if the parameters are
		...
		/// <summary>
		/// Sets the cache policy.  Unless a handler overrides this method, handlers will not allow a response to be cached.
		/// </summary>
		...
		{
			// It's really too bad that StatusCode property is not of type HttpStatusCode.
			context.Response.StatusCode = (int)HttpStatusCode.InternalServerError;
			context.Response.End();
		}

		/// <summary>
		/// Helper method used to respond to the request that the request is attempting to access a resource that the user
